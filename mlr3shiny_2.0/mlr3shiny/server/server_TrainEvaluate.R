...
output$download1 <- downloadHandler(
  filename = function() {
    paste0("Pred_Train(",format(Sys.time(), "%H-%M-%S"),").xlsx")
  },
  content = function(file) {
    write.xlsx(data.frame(id=Wf$Pred_Train$data$row_ids,truth=Wf$Pred_Train$data$truth,response=Wf$Pred_Train$data$response), file)
  }
)
output$download2 <- downloadHandler(
  filename = function() {
    paste0("Pred_Test(",format(Sys.time(), "%H-%M-%S"),").xlsx")
  },
  content = function(file) {
    write.xlsx(data.frame(id=Wf$Pred_Test$data$row_ids,truth=Wf$Pred_Test$data$truth,response=Wf$Pred_Test$data$response), file)
  }
)
